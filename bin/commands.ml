...
let with_initialized_db ?(table = Runner.default_table) database_url
    (f : Types.db_conn -> int Lwt.t) : int Lwt.t =
  ...
  | Ok () -> (
      ...
      | Ok dialect -> (
          Database.with_db database_url (fun db ->
              Lwt.bind (Runner.ensure_migrations_table ~table dialect db)
                (function
                | Error err ->
...
let run_migration_with_progress ?(verbose = false)
    ?(table = Runner.default_table) db (migration : Migration.t) :
    Runner.execution_result Lwt.t =
  ...
  | Runner.Failure (migration, err) ->
      Lwt_io.eprintlf "** Migration %Ld failed: %s" migration.version
        (Types.show_error err)
      >>= fun () -> Lwt.return result

let rollback_migration_with_progress ?(verbose = false)
    ?(table = Runner.default_table) db (migration : Migration.t) :
    Runner.execution_result Lwt.t =
  let filename = migration_filename migration in
  ...
  | Runner.Success migration ->
      ...
      >>= fun () -> Lwt.return result
  | Runner.Failure (migration, err) ->
      Lwt_io.eprintlf "** Rollback %Ld failed: %s" migration.version
        (Types.show_error err)
      >>= fun () -> Lwt.return result

(** Run multiple migrations with progress, stopping on first failure. Shares
    Runner's sequential engine; the progress-printing per-migration action is
    the [step]. *)
let run_migrations_with_progress ?(verbose = false)
    ?(table = Runner.default_table) db migrations :
    Runner.execution_result list Lwt.t =
  Runner.run_until_failure
    ~step:(run_migration_with_progress ~verbose ~table db)
    ~is_ok:Runner.is_success migrations

let rollback_migrations_with_progress ?(verbose = false)
    ?(table = Runner.default_table) db migrations :
    Runner.execution_result list Lwt.t =
  let sorted =
    List.sort
  ...
  Runner.run_until_failure
    ...
    ~is_ok:Runner.is_success sorted

let code_of_results results =
  if List.exists (fun r -> not (Runner.is_success r)) results then 1 else 0

(** Print the database dialect when [verbose]; never fails (errors surface
    later). *)
...
let generate name =
  ...
  | Ok () ->
      ...
      if Sys.file_exists filepath then
        Lwt_io.eprintlf "Error: migration file already exists: %s" filepath
        >>= fun () -> Lwt.return 1
      else
        let template = "-- +migrate up\n\n\n-- +migrate down\n\n" in
        Lwt_io.with_file ~mode:Lwt_io.Output filepath (fun oc ->
            Lwt_io.write oc template)
        >>= fun () ->
        Lwt_io.printlf "Creating %s" filepath >>= fun () -> Lwt.return 0

let print_plan verb (migrations : Migration.t list) =
  Lwt_io.printlf "Would %s %d migration(s):" verb (List.length migrations)
  >>= fun () ->
  Lwt_list.iter_s
    (fun (m : Migration.t) ->
      Lwt_io.printlf "  %Ld  %s" m.Migration.version m.Migration.description)
    migrations
  >>= fun () -> Lwt.return 0
...
let migrate migrations_dir table dry_run verbose database_url =
  ...
  with_initialized_db ~table database_url (fun db ->
      ...
      | Ok () -> (
          ...
          | Ok pending ->
              run_migrations_with_progress ~verbose ~table db pending
              >>= fun results -> Lwt.return (code_of_results results)))
  ...
  | Ok db_name -> (
      ...
      | Ok () ->
          ...
          with_initialized_db ~table database_url (fun db ->
              Runner.pending_migrations ~table ~migrations_dir db >>= function
              | Error err ->
                  Lwt_io.eprintlf "Error: %s" (Types.show_error err)
                  >>= fun () -> Lwt.return 1
              | Ok [] ->
  ...
  | Ok db_name -> (
      ...
      | Ok () -> (
          ...
          | Ok () ->
              ...
              with_initialized_db ~table database_url (fun db ->
                  Runner.pending_migrations ~table ~migrations_dir db
                  >>= function
                  | Error err ->
                      Lwt_io.eprintlf "Error: %s" (Types.show_error err)
                      >>= fun () -> Lwt.return 1
...
let rollback migrations_dir table step to_version all dry_run verbose
  ...
  with_initialized_db ~table database_url (fun db ->
      ...
      | Ok to_rollback ->
          rollback_migrations_with_progress ~verbose ~table db to_rollback
          >>= fun results -> Lwt.return (code_of_results results))
...
let redo migrations_dir table step verbose database_url =
  ...
  with_initialized_db ~table database_url (fun db ->
      Runner.rollback_targets ~table ~migrations_dir db (Runner.Step n)
      >>= function
      | Error err ->
          Lwt_io.eprintlf "Error: %s" (Types.show_error err) >>= fun () ->
          Lwt.return 1
      ...
      | Ok targets -> (
          rollback_migrations_with_progress ~verbose ~table db targets
          >>= fun rolled_back ->
  ...
  with_initialized_db ~table database_url (fun db ->
      ...
      | Ok applied_versions -> (
          ...
          | Ok migrations ->
              ...
              >>= fun () ->
              (* Print table header *)
              Lwt_io.printl "  Status    Migration ID    Migration Name"
              >>= fun () ->
              Lwt_io.printl "--------------------------------------------------"
              >>= fun () ->
              ...
              (match migrations with
                ...
                | _ ->
                    Lwt_list.iter_s
                      (fun migration ->
                        let is_applied =
                          Discovery.Int64Set.mem migration.Migration.version
                            applied_set
                        in
                        let status = if is_applied then "up" else "down" in
                        Lwt_io.printlf "  %-8s  %Ld  %s" status
