...
let db_url_arg =
  Arg.(value & opt (some string) None & info ["D"; "database-url"] ~docv:"URL"
    ~doc:"Connection URL (overrides the DATABASE_URL environment variable)")

(* Shared --dry-run flag *)
let dry_run_arg =
  Arg.(value & flag & info ["dry-run"] ~doc:"Show what would happen without changing the database")
...
(* Shared --step option *)
let step_arg =
  Arg.(value & opt (some int) None & info ["step"] ~docv:"N" ~doc:"Number of migrations (default: 1)")

(* Migrate command *)
let migrate_cmd =
  let run migrations_dir table dry_run verbose db_url =
    run_lwt (fun () -> Commands.migrate migrations_dir table dry_run verbose (resolve_database_url db_url))
  in
  let info = Cmd.info "migrate" ~doc:"Run all pending migrations" in
  Cmd.v info Term.(const run $ migrations_dir_arg $ table_arg $ dry_run_arg $ verbose_arg $ db_url_arg)

(* Init command *)
let init_cmd =
  let run db_url = run_lwt (fun () -> Commands.init (resolve_database_url db_url)) in
...
let setup_cmd =
  let run migrations_dir table verbose db_url =
    run_lwt (fun () -> Commands.setup migrations_dir table verbose (resolve_database_url db_url))
  in
  let info = Cmd.info "setup" ~doc:"Create the database and run migrations" in
  Cmd.v info Term.(const run $ migrations_dir_arg $ table_arg $ verbose_arg $ db_url_arg)

(* Drop command *)
let drop_cmd =
  let run db_url = run_lwt (fun () -> Commands.drop (resolve_database_url db_url)) in
...
let reset_cmd =
  let run migrations_dir table verbose db_url =
    run_lwt (fun () -> Commands.reset migrations_dir table verbose (resolve_database_url db_url))
  in
  let info = Cmd.info "reset" ~doc:"Drop the database and recreate it with migrations" in
  Cmd.v info Term.(const run $ migrations_dir_arg $ table_arg $ verbose_arg $ db_url_arg)
...
let rollback_cmd =
  ...
  in
  let all = Arg.(value & flag & info ["all"] ~doc:"Rollback all migrations") in
  let run migrations_dir table step to_version all dry_run verbose db_url =
    run_lwt (fun () ->
      Commands.rollback migrations_dir table step to_version all dry_run verbose (resolve_database_url db_url))
  in
  let info = Cmd.info "rollback" ~doc:"Rollback migrations" in
  Cmd.v info Term.(const run $ migrations_dir_arg $ table_arg $ step_arg $ to_version $ all $ dry_run_arg $ verbose_arg $ db_url_arg)

(* Redo command *)
let redo_cmd =
  let run migrations_dir table step verbose db_url =
    run_lwt (fun () -> Commands.redo migrations_dir table step verbose (resolve_database_url db_url))
  in
  let info = Cmd.info "redo" ~doc:"Roll back the last migration(s) and re-apply them" in
  Cmd.v info Term.(const run $ migrations_dir_arg $ table_arg $ step_arg $ verbose_arg $ db_url_arg)

(* Status command *)
let status_cmd =
  let run migrations_dir table db_url =
    run_lwt (fun () -> Commands.status migrations_dir table (resolve_database_url db_url))
  in
...
let main_cmd =
  ...
  Cmd.group info [generate_cmd; drop_cmd; init_cmd; migrate_cmd; redo_cmd; reset_cmd; rollback_cmd; setup_cmd; status_cmd]
