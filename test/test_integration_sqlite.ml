...
let with_sqlite_file prefix f =
  ...
  let db_file = Printf.sprintf "/tmp/migra_test_%s_%d_%d.db" prefix timestamp random in
...
let test_sqlite_file_creation () =
  with_sqlite_file "file_create" (fun db_url db_file ->
    Alcotest.(check bool) "SQLite file doesn't exist initially" false (Sys.file_exists db_file);

    with_db db_url (fun db ->
      Alcotest.(check bool) "SQLite file created on connect" true (Sys.file_exists db_file);
  ...
  with_sqlite_file "create_db" (fun db_url db_file ->
    ...
    | Ok () ->
        with_db db_url (fun _db ->
          Alcotest.(check bool) "SQLite file exists after connect" true (Sys.file_exists db_file);
          Lwt.return_unit
        )
  )
...
let test_sqlite_drop_database () =
  with_sqlite_file "drop_db" (fun db_url db_file ->
    ...
    | Ok () ->
        Alcotest.(check bool) "SQLite file deleted after drop" false (Sys.file_exists db_file);
        Lwt.return_unit
  )
    ...
    with_db db_url (fun db ->
      ...
      | Ok () ->
          ...
          | Error err ->
              Alcotest.fail (Printf.sprintf "add_migration failed: %s" (Caqti_error.show err))
          | Ok () ->
              Migra.Runner.get_applied_records Migra.Dialect.SQLite db >>= function
              | Error err ->
      ...
      | Ok () ->
          ...
          | Ok () ->
              ...
              | Error err ->
                  Alcotest.fail (Printf.sprintf "is_applied failed: %s" (Caqti_error.show err))
              | Ok applied ->
                  Alcotest.(check bool) "Migration applied in memory" true applied;
                  Lwt.return_unit
    )
  )
...
let test_sqlite_memory_create_database () =
  ...
  Migra.Database.create_database db_url >>= function
  | Error err ->
      Alcotest.fail (Printf.sprintf "create_database for :memory: failed: %s" (Migra.Types.show_error err))
  | Ok () ->
      Lwt.return_unit
...
let test_sqlite_memory_drop_database () =
  ...
  Migra.Database.drop_database db_url >>= function
  | Error err ->
      Alcotest.fail (Printf.sprintf "drop_database for :memory: failed: %s" (Migra.Types.show_error err))
  | Ok () ->
      Lwt.return_unit
...
let file_based_tests = [
  ...
  "SQLite create_database (no-op for file)", `Quick, test_sqlite_create_database;
