  ...
  with_test_db_pooled "schema_create" (fun db_url ->
      with_db db_url (fun db ->
          ...
          | Ok () -> (
              ...
              | Error err ->
                  Alcotest.fail
                    (Printf.sprintf "Query after create failed: %s"
                       (Caqti_error.show err))
              | Ok versions ->
                  Alcotest.(check int)
                    "New table is empty" 0 (List.length versions);
                  Lwt.return_unit)))
...
let test_create_table_idempotent () =
  with_test_db_pooled "schema_idempotent" (fun db_url ->
      with_db db_url (fun db ->
          ...
          | Error err ->
              Alcotest.fail
                (Printf.sprintf "First create_table failed: %s"
                   (Caqti_error.show err))
          | Ok () -> (
              Migra.Runner.ensure_migrations_table Migra.Dialect.PostgreSQL db
              >>= function
              | Error err ->
                  Alcotest.fail
                    (Printf.sprintf "Second create_table failed: %s"
                       (Caqti_error.show err))
              | Ok () -> Lwt.return_unit)))
...
let test_initialize_idempotent () =
  with_test_db_pooled "schema_init" (fun db_url ->
      with_db db_url (fun db ->
          ...
          | Error err ->
              Alcotest.fail
                (Printf.sprintf "First initialize failed: %s"
                   (Caqti_error.show err))
          | Ok () -> (
              Migra.Runner.ensure_migrations_table Migra.Dialect.PostgreSQL db
              >>= function
              | Error err ->
                  Alcotest.fail
                    (Printf.sprintf "Second initialize failed: %s"
                       (Caqti_error.show err))
              | Ok () -> Lwt.return_unit)))
...
let test_is_applied_false () =
  with_test_db_pooled "schema_is_applied_false" (fun db_url ->
      with_db db_url (fun db ->
          ...
          | Error err ->
              Alcotest.fail
                (Printf.sprintf "Initialize failed: %s" (Caqti_error.show err))
          | Ok () -> (
              Migra.Runner.is_applied db 20240115120000L >>= function
              | Error err ->
                  Alcotest.fail
      ...
      with_db db_url (fun db ->
          ...
          | Error err ->
              Alcotest.fail
                (Printf.sprintf "Initialize failed: %s" (Caqti_error.show err))
          | Ok () -> (
              Migra.Runner.get_applied_records Migra.Dialect.PostgreSQL db
              >>= function
              | Error err ->
                  Alcotest.fail
              ...
              | Ok records ->
                  Alcotest.(check int)
                    "Empty records list" 0 (List.length records);
                  Lwt.return_unit)))
      ...
      with_db db_url (fun db ->
          ...
          | Ok () -> (
              ...
              | Ok () -> (
                  ...
                  | Ok applied ->
                      Alcotest.(check bool)
                        "Migration was recorded" true applied;
                      Lwt.return_unit))))
...
let test_remove_migration_success () =
  with_test_db_pooled "schema_remove" (fun db_url ->
      with_db db_url (fun db ->
          ...
          | Ok () -> (
              let version = 20240115120000L in
              Migra.Runner.add_migration db version "test-checksum" >>= fun _ ->
              ...
              | Ok () -> (
                  ...
                  | Ok applied ->
                      Alcotest.(check bool)
                        "Migration was removed" false applied;
                      Lwt.return_unit))))
      ...
      with_db db_url (fun db ->
          ...
          | Error err ->
              Alcotest.fail
                (Printf.sprintf "Initialize failed: %s" (Caqti_error.show err))
          | Ok () -> (
              Migra.Runner.remove_migration db 20240115120000L >>= function
              | Error err ->
                  Alcotest.fail
                    (Printf.sprintf "remove_migration failed: %s"
                       (Caqti_error.show err))
  ...
  with_test_db_pooled "schema_latest_some" (fun db_url ->
      with_db db_url (fun db ->
          ...
          | Ok () -> (
              ...
              | Ok latest ->
                  Alcotest.(check (option int64_testable))
                    "Latest version" (Some version) latest;
                  Lwt.return_unit)))
...
let suite =
  [
    ...
    ( "is_applied returns false for non-existent version",
      `Quick,
      test_is_applied_false );
    ("is_applied returns true for applied version", `Quick, test_is_applied_true);
    ( "get_applied_versions returns empty list for new table",
      `Quick,
      test_get_applied_versions_empty );
    ( "get_applied_versions returns sorted list",
      `Quick,
      test_get_applied_versions_sorted );
    ( "get_applied_records returns empty list for new table",
      `Quick,
      test_get_applied_records_empty );
    ( "get_applied_records returns records with timestamps",
      `Quick,
      test_get_applied_records_with_timestamps );
    ( "add_migration successfully inserts version",
      `Quick,
      test_add_migration_success );
    ( "add_migration fails on duplicate version",
      `Quick,
      test_add_migration_duplicate_fails );
    ( "remove_migration successfully removes version",
      `Quick,
      test_remove_migration_success );
    ( "remove_migration is safe on non-existent version",
      `Quick,
      test_remove_migration_nonexistent );
    ( "get_latest_version returns None for empty table",
      `Quick,
      test_get_latest_version_none );
    ( "get_latest_version returns Some(version) for non-empty table",
      `Quick,
      test_get_latest_version_some );
