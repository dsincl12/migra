...
let test_run_migration_success () =
  with_test_db_pooled "runner_success" (fun db_url ->
    with_temp_dir "migrations" (fun migrations_dir ->
      ...
      let _filepath = create_migration_with_sections migrations_dir version "create_table"
      ...
      with_initialized_db db_url (fun db ->
        ...
        (match result with
         ...
         | Migris.Runner.Success _migration ->
             ...
             | Ok is_applied ->
                 Alcotest.(check bool) "version recorded in schema_migrations" true is_applied;

                 let module Db = (val db : Caqti_lwt.CONNECTION) in
                 let open Caqti_request.Infix in
                 let open Caqti_type.Std in
                 let query = (unit ->! bool) "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'test_users')" in
                 Db.find query () >>= function
                 | Error err ->
                     Alcotest.fail (Printf.sprintf "Table existence check failed: %s" (Caqti_error.show err))
                 | Ok table_exists ->
                     Alcotest.(check bool) "test_users table exists" true table_exists;
                     Lwt.return_unit)
      )
    )
  )
...
let test_run_migration_sql_failure_rollback () =
  with_test_db_pooled "runner_sql_fail" (fun db_url ->
    with_temp_dir "migrations" (fun migrations_dir ->
      let version = 20240115120000L in
      let _filepath = create_migration_with_sections migrations_dir version "invalid_sql"
        ...
        (match result with
         ...
         | Migris.Runner.Failure (_migration, _err) ->
             ...
             | Error err ->
                 Alcotest.fail (Printf.sprintf "is_applied check failed: %s" (Caqti_error.show err))
             | Ok is_applied ->
                 Alcotest.(check bool) "version NOT recorded (transaction rolled back)" false is_applied;

                 let module Db = (val db : Caqti_lwt.CONNECTION) in
                 let open Caqti_request.Infix in
                 let open Caqti_type.Std in
                 let query = (unit ->! bool) "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'test_table')" in
                 Db.find query () >>= function
                 | Error err ->
                     Alcotest.fail (Printf.sprintf "Table existence check failed: %s" (Caqti_error.show err))
                 | Ok table_exists ->
                     Alcotest.(check bool) "test_table does NOT exist (rolled back)" false table_exists;
                     Lwt.return_unit)
      )
    )
  )
  ...
  with_test_db_pooled "runner_multiple" (fun db_url ->
    with_temp_dir "migrations" (fun migrations_dir ->
      ...
      with_initialized_db db_url (fun db ->
        ...
        | Ok versions ->
            Alcotest.(check int) "3 versions recorded" 3 (List.length versions);
            ...
            Alcotest.(check int64_testable) "v3 recorded" v3 (List.nth versions 2);
            Lwt.return_unit
      )
    )
  )
...
let test_run_migrations_stops_on_failure () =
  with_test_db_pooled "runner_stop_fail" (fun db_url ->
    with_temp_dir "migrations" (fun migrations_dir ->
      ...
      let _f1 = create_migration_with_sections migrations_dir v1 "table1"
        "CREATE TABLE table1 (id SERIAL PRIMARY KEY);" "DROP TABLE table1;" in
      let _f2 = create_migration_with_sections migrations_dir v2 "invalid"
      ...
      with_initialized_db db_url (fun db ->
        ...
        | Error err ->
            Alcotest.fail (Printf.sprintf "get_applied_versions failed: %s" (Caqti_error.show err))
        | Ok versions ->
            Alcotest.(check int) "only 1 version recorded" 1 (List.length versions);
            Alcotest.(check int64_testable) "v1 recorded" v1 (List.nth versions 0);
            Lwt.return_unit
      )
    )
            ...
            (match result with
             ...
             | Migris.Runner.Success _ ->
                 ...
                 | Error err ->
                     Alcotest.fail (Printf.sprintf "is_applied check failed: %s" (Caqti_error.show err))
                 | Ok is_applied ->
                     Alcotest.(check bool) "version removed" false is_applied;

                     let module Db = (val db : Caqti_lwt.CONNECTION) in
                     let open Caqti_request.Infix in
                     let open Caqti_type.Std in
                     let query = (unit ->! bool) "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'test_rollback')" in
                     Db.find query () >>= function
                     | Error err ->
                         Alcotest.fail (Printf.sprintf "Table existence check failed: %s" (Caqti_error.show err))
                     | Ok table_exists ->
                         Alcotest.(check bool) "test_rollback table dropped" false table_exists;
                         Lwt.return_unit)
      )
    )
  )
...
let test_rollback_migration_sql_failure () =
  with_test_db_pooled "runner_rollback_fail" (fun db_url ->
    with_temp_dir "migrations" (fun migrations_dir ->
      let version = 20240115120000L in
      let _filepath = create_migration_with_sections migrations_dir version "bad_rollback"
      ...
      with_initialized_db db_url (fun db ->
        ...
        | Migris.Runner.Success _ ->
            ...
            | Error err ->
                Alcotest.fail (Printf.sprintf "is_applied check failed: %s" (Caqti_error.show err))
            | Ok is_applied ->
                Alcotest.(check bool) "version still recorded (rollback rolled back)" true is_applied;
                Lwt.return_unit
      )
    )
  )
      ...
      with_initialized_db db_url (fun db ->
        ...
        | Ok results ->
            ...
            | Error err ->
                Alcotest.fail (Printf.sprintf "get_applied_versions failed: %s" (Caqti_error.show err))
            | Ok versions ->
                Alcotest.(check int) "only 1 version remains" 1 (List.length versions);
...
let suite = [
  ...
  "run_migration_sql_failure_rollback", `Quick, test_run_migration_sql_failure_rollback;
  "run_migration_file_error", `Quick, test_run_migration_file_error;
  "run_migrations_multiple", `Quick, test_run_migrations_multiple;
  "run_migrations_stops_on_failure", `Quick, test_run_migrations_stops_on_failure;
  "rollback_migration_success", `Quick, test_rollback_migration_success;
  "rollback_migration_sql_failure", `Quick, test_rollback_migration_sql_failure;
