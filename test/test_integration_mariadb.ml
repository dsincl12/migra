...
let create_mariadb_test_db prefix =
  ...
  | Error err ->
      Lwt.return_error (Printf.sprintf "Failed to connect to MariaDB admin: %s" (Migra.Types.show_error err))
  | Ok db ->
      let module Db = (val db : Caqti_lwt.CONNECTION) in
      let open Caqti_request.Infix in
      let open Caqti_type.Std in

      let query = (unit ->. unit) ~oneshot:true
        (Printf.sprintf "CREATE DATABASE IF NOT EXISTS `%s`" db_name) in

      Db.exec query () >>= function
      | Error err ->
          Lwt.return_error (Printf.sprintf "Failed to create test DB: %s" (Caqti_error.show err))
      | Ok () ->
...
let drop_mariadb_test_db db_name =
  ...
  | Ok db ->
      ...
      let query = (unit ->. unit) ~oneshot:true
        (Printf.sprintf "DROP DATABASE IF EXISTS `%s`" db_name) in

      Db.exec query () >>= function
      | Error err ->
          Lwt.return_error (Printf.sprintf "Failed to drop test DB: %s" (Caqti_error.show err))
      | Ok () ->
          Lwt.return_ok ()
...
let test_mariadb_create_database () =
  ...
  Lwt.finalize
    (fun () ->
      ...
      | Error err ->
          Alcotest.fail (Printf.sprintf "create_database failed: %s" (Migra.Types.show_error err))
      | Ok () ->
    ...
    (fun () ->
      drop_mariadb_test_db db_name >>= fun _ -> Lwt.return_unit
    )
...
let test_mariadb_create_database_idempotent () =
  ...
  Lwt.finalize
    (fun () ->
      Migra.Database.create_database db_url >>= function
      | Error err ->
          Alcotest.fail (Printf.sprintf "First create_database failed: %s" (Migra.Types.show_error err))
      | Ok () ->
          Migra.Database.create_database db_url >>= function
          | Error err ->
              Alcotest.fail (Printf.sprintf "Second create_database failed: %s" (Migra.Types.show_error err))
          | Ok () ->
              Lwt.return_unit
    )
    (fun () ->
  ...
  Lwt.finalize
    (fun () ->
      ...
      | Ok () ->
          ...
          | Error err ->
              Alcotest.fail (Printf.sprintf "drop_database failed: %s" (Migra.Types.show_error err))
          | Ok () ->
              Migra.Database.connect_db db_url >>= function
              | Ok _db ->
                  Alcotest.fail "Database still exists after drop"
              | Error _ ->
                  Lwt.return_unit
    )
    (fun () ->
...
let test_mariadb_drop_database_idempotent () =
  ...
  Lwt.finalize
    (fun () ->
      Migra.Database.drop_database db_url >>= function
      | Error err ->
          Alcotest.fail (Printf.sprintf "drop_database on non-existent DB failed: %s" (Migra.Types.show_error err))
      | Ok () ->
          Lwt.return_unit
    )
    (fun () ->
  ...
  with_mariadb_test_db "schema_innodb" (fun db_url ->
    with_db db_url (fun db ->
      ...
      | Ok () ->
          ...
          | Error err ->
              Alcotest.fail (Printf.sprintf "Failed to check engine: %s" (Caqti_error.show err))
          | Ok engine ->
...
let test_mariadb_schema_idempotent () =
  with_mariadb_test_db "schema_idem" (fun db_url ->
    with_db db_url (fun db ->
      Migra.Runner.ensure_migrations_table Migra.Dialect.MariaDB db >>= function
      | Error err ->
          Alcotest.fail (Printf.sprintf "First ensure_migrations_table failed: %s" (Caqti_error.show err))
      | Ok () ->
          Migra.Runner.ensure_migrations_table Migra.Dialect.MariaDB db >>= function
          | Error err ->
              Alcotest.fail (Printf.sprintf "Second ensure_migrations_table failed: %s" (Caqti_error.show err))
          | Ok () ->
              Lwt.return_unit
    )
  )
      ...
      | Ok () ->
          ...
          | Ok applied ->
              ...
              | Ok () ->
                  Migra.Runner.is_applied db version >>= function
                  | Error err ->
                      Alcotest.fail (Printf.sprintf "is_applied after add failed: %s" (Caqti_error.show err))
                  | Ok applied ->
                      Alcotest.(check bool) "Version applied after add" true applied;
                      ...
                      | Ok () ->
                          Migra.Runner.is_applied db version >>= function
                          | Error err ->
                              Alcotest.fail (Printf.sprintf "is_applied after remove failed: %s" (Caqti_error.show err))
                          | Ok applied ->
                              Alcotest.(check bool) "Version not applied after remove" false applied;
                              Lwt.return_unit
    )
  )
    ...
    with_db db_url (fun db ->
      ...
      | Ok () ->
          ...
          | Error err ->
              Alcotest.fail (Printf.sprintf "add_migration failed: %s" (Caqti_error.show err))
          | Ok () ->
              Migra.Runner.get_applied_records Migra.Dialect.MariaDB db >>= function
              | Error err ->
              ...
              | Ok records ->
                  ...
                  Alcotest.(check int64_testable) "Record version" version record.Migra.Runner.version;
                  Alcotest.(check bool) "created_at exists"
                    true (String.length record.Migra.Runner.created_at > 0);
                  Lwt.return_unit
    )
  )
  ...
  with_mariadb_test_db "latest_version" (fun db_url ->
    with_db db_url (fun db ->
      ...
      | Ok () ->
          ...
          | Ok latest ->
              Alcotest.(check (option int64_testable)) "No latest version initially" None latest;
      ...
      | Ok () ->
          ...
          | Ok () ->
              ...
              | Ok records ->
                  Alcotest.(check int) "One record" 1 (List.length records);
                  let record = List.hd records in
                  Alcotest.(check bool) "Timestamp converted to string"
                    true (String.length record.Migra.Runner.created_at > 0);
                  Lwt.return_unit
    )
  )
...
let database_lifecycle_tests = [
  ...
  "MariaDB create_database is idempotent", `Quick, test_mariadb_create_database_idempotent;
  "MariaDB drop_database", `Quick, test_mariadb_drop_database;
  "MariaDB drop_database is idempotent", `Quick, test_mariadb_drop_database_idempotent;
]
...
let migration_tests = [
  ...
  "MariaDB duplicate migration fails", `Quick, test_mariadb_duplicate_migration_fails;
]
