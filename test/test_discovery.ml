...
let test_find_migrations () =
  Lwt_main.run (
    with_temp_dir "discovery_test" (fun dir ->
      let _ = create_migration_with_sections dir 20240115130000L "third" "CREATE TABLE c;" "DROP TABLE c;" in
...
let test_find_migrations_filters () =
  Lwt_main.run (
    with_temp_dir "discovery_filter" (fun dir ->
      let _ = create_migration_with_sections dir 20240115120000L "valid" "CREATE TABLE t;" "DROP TABLE t;" in
  ...
  Lwt_main.run (
    with_temp_dir "discovery_ensure" (fun dir ->
      ...
      Alcotest.(check bool) "dir exists"
        true (Sys.file_exists migrations_dir);
      Alcotest.(check bool) "is a directory"
        true (Sys.is_directory migrations_dir);
...
let suite = [
  "is_migration_file_valid", `Quick, async_of_sync test_is_migration_file_valid;
  "is_migration_file_invalid", `Quick, async_of_sync test_is_migration_file_invalid;
  ...
  "find_pending_all_applied", `Quick, async_of_sync test_find_pending_all_applied;
  ...
  "find_by_version_not_found", `Quick, async_of_sync test_find_by_version_not_found;
  "ensure_migrations_dir_creates", `Quick, async_of_sync test_ensure_migrations_dir_creates;
  "ensure_migrations_dir_idempotent", `Quick, async_of_sync test_ensure_migrations_dir_idempotent;
  "ensure_migrations_dir_file_exists", `Quick, async_of_sync test_ensure_migrations_dir_file_exists;
]
