...
let test_get_admin_database_url_with_user () =
  ...
  let result =
    Migra.Database.get_admin_database_url Migra.Dialect.PostgreSQL uri
  in
  Alcotest.(check bool) "build succeeds" true (is_ok result);
  let admin_url = get_ok result in
  Alcotest.(check string)
    "admin URL correct" "postgresql://myuser@localhost:5432/postgres" admin_url;
  Lwt.return_unit
...
let test_get_admin_database_url_no_user () =
  ...
  let admin_url = get_ok result in
  Alcotest.(check string)
    "admin URL correct" "postgresql://localhost:5432/postgres" admin_url;
  Lwt.return_unit
...
let test_get_admin_database_url_with_password () =
  ...
  (* The password must be preserved: admin commands (init/setup/drop/reset)
     ...
     if it is stripped. *)
  Alcotest.(check string)
    "admin URL keeps user and password"
...
let test_get_admin_database_url_default_port () =
  ...
  let admin_url = get_ok result in
  Alcotest.(check string)
    "admin URL uses default port" "postgresql://myuser@localhost:5432/postgres"
    admin_url;
  Lwt.return_unit
...
let test_create_database_rejects_slash () =
  ...
  | Error err ->
      ...
      Alcotest.(check bool)
        "mentions the bad name" true
        (contains_sub msg "db/extra");
      Alcotest.(check bool)
        "explains the rule" true
        (contains_sub msg "cannot contain");
      Lwt.return_unit
...
let test_with_db () =
  with_test_db_pooled "db_with" (fun db_url ->
      Migra.Database.with_db db_url (fun _db -> Lwt.return 42) >>= function
      | Error msg ->
          Alcotest.fail
...
let test_with_db_exception () =
  with_test_db_pooled "db_with_exception" (fun db_url ->
      ...
      | Error err ->
          let msg = Migra.Types.show_error err in
          Alcotest.(check bool)
            "error message contains failure" true
            (String.length msg > 0);
          Lwt.return_unit)
  ...
  with_test_db_pooled "db_initialize" (fun db_url ->
      ...
      | Ok db -> (
          ...
          | Ok () -> (
              ...
              | Ok versions ->
                  Alcotest.(check int)
                    "schema_migrations table is empty" 0 (List.length versions);
                  Lwt.return_unit)))
...
let suite =
  [
    ...
    ( "get_admin_database_url_with_user",
      `Quick,
      test_get_admin_database_url_with_user );
    ( "get_admin_database_url_no_user",
      `Quick,
      test_get_admin_database_url_no_user );
    ( "get_admin_database_url_with_password",
      `Quick,
      test_get_admin_database_url_with_password );
    ( "get_admin_database_url_default_port",
      `Quick,
      test_get_admin_database_url_default_port );
