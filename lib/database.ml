...
(** Check if [haystack] contains [needle].
    Pure scan - avoids the [Str] library, whose global match state is not
    safe under Lwt's concurrent scheduling. *)
...
let missing_driver_message (database_url : string) : string =
  ...
  let driver_name, install_cmd = match scheme with
    | "postgresql" | "postgres" -> ("PostgreSQL", "opam install caqti-driver-postgresql")
    | "mariadb" | "mysql" -> ("MariaDB/MySQL", "opam install caqti-driver-mariadb")
...
let connect_db (database_url : string) : ((Types.db_conn, Types.error) result) Lwt.t =
  ...
  | Error err ->
      if is_missing_driver_error (Caqti_error.show err) then
        Error (Types.DatabaseError (Types.ValidationError (missing_driver_message database_url)))
      else if likely_unencoded_credentials database_url then
        Error (Types.DatabaseError (Types.ValidationError
          (Printf.sprintf
             "%s\n\nHint: the connection URL contains more than one '@'. If your \
              username or password contains '@' (or ':' '/' '?' '#'), percent-encode \
              it - e.g. '@' becomes '%%40' - otherwise it is misread as the host."
...
(** Connect to the admin database for [database_url]'s dialect and run [f] with
    the connection and the target database name, disconnecting afterwards.
    For server dialects only (SQLite has no admin database). *)
let with_admin_connection (dialect : Dialect.t) (database_url : string)
  ...
  | Ok db_name when String.contains db_name '/' ->
      ...
      Lwt.return_error (Types.DatabaseError (Types.UrlParseError
        (Printf.sprintf
           "Invalid database name %S: a database name cannot contain '/' \
            (check the path in your DATABASE_URL)" db_name)))
  | Ok db_name ->
      ...
      | Ok admin_url ->
          ...
          | Ok db ->
              ...
              Lwt.finalize (fun () -> f db db_name) (fun () -> Conn.disconnect ())
